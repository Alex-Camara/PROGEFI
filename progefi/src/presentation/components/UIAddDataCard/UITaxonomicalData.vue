...
export default {
  ...
  computed: {
    ...mapState("speciesData", {
      ...
      isScientificNameValid: state => state.scientificName.valid,
      isCommonNameValid: state => state.commonName.valid,
      isGenusValid: state => state.genus.valid,
      isFamilyValid: state => state.family.valid,
      isOrderValid: state => state.order.valid,
      isPhylumValid: state => state.phylum.valid,
      isSpeciesClassValid: state => state.speciesClass.valid,
      isKingdomValid: state => state.kingdom.valid,
    ...
    scientificName: {
      get: function() {
        let scientificName = this.speciesDataState.scientificName;
        if (
          !scientificName.valid.isValid ||
          scientificName.valid.message == "temporary error"
        ) {
    ...
    commonName: {
      get: function() {
        let commonName = this.speciesDataState.commonName;
        if (
          !commonName.valid.isValid ||
          commonName.valid.message == "temporary error"
        ) {
    ...
    genus: {
      get: function() {
        let genus = this.speciesDataState.genus;
        if (!genus.valid.isValid || genus.valid.message == "temporary error") {
    ...
    order: {
      get: function() {
        let order = this.speciesDataState.order;
        if (!order.valid.isValid || order.valid.message == "temporary error") {
    ...
    family: {
      get: function() {
        let family = this.speciesDataState.family;
        if (
          !family.valid.isValid ||
          family.valid.message == "temporary error"
        ) {
    ...
    speciesClass: {
      get: function() {
        let speciesClass = this.speciesDataState.speciesClass;
        if (
          !speciesClass.valid.isValid ||
          speciesClass.valid.message == "temporary error"
        ) {
          this.addShakeEffect("species_class_input");
        }
        return speciesClass.name;
      },
      set: function(newValue) {
    ...
    phylum: {
      get: function() {
        let phylum = this.speciesDataState.phylum;
        if (
          !phylum.valid.isValid ||
          phylum.valid.message == "temporary error"
        ) {
    ...
    kingdom: {
      get: function() {
        let kingdom = this.speciesDataState.kingdom;
        if (
          !kingdom.valid.isValid ||
          kingdom.valid.message == "temporary error"
        ) {
